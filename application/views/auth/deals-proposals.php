				...
				<div class="accordion" id="accordionExample">
									...
									<form id="DealForm">
										...
										<div class="form-row mb-3">
											...
											<div class="col-md-2">
												...
												<input type="number" class="form-control shadow-sm" id="mobile_num" name="mobile_num" placeholder="Mobile Number" required>
											</div>
											<div class="col-md-2">
												<label for="phone" class="form-label">Phone:</label>
												<input type="number" class="form-control shadow-sm" id="phone" name="phone" placeholder="Phone Number" required>
											</div>
											<div class="col-md-3">
											...
											<div class="col-md-3">
												<label for="p_box" class="form-label">P.BOX:</label>
												<input type="text" class="form-control shadow-sm" id="p_box" name="p_box" placeholder="P.BOX Number" required>
											</div>
											<div class="col-md-2">
												<label for="trn" class="form-label">TRN:</label>
												<input type="number" class="form-control shadow-sm" id="trn" name="trn" placeholder="TRN" required>
											</div>
										</div>
					...
					<div class="card shadow-sm">
						<div class="card-header text-white" id="headingFour" style="background-color: #d10908;">
							<h2 class="mb-0">
								<button class="btn btn-link text-white collapsed" type="button" data-toggle="collapse" data-target="#collapseFour" aria-expanded="false" aria-controls="collapseFour" style="font-weight: bold; text-decoration: none;">
									<i id="addItemIcon" class="bi bi-plus-circle me-2"></i>Create New Item
								</button>
							</h2>
						</div>
								...
								<form id="ItemForm">
									<div class="form-row mb-3">
										<div class="col-md-12">
											...
											<input type="text" class="form-control shadow-sm" id="item_name" name="item_name" placeholder="Enter Item Name" required>
										</div>
									</div>
									...
									<div class="form-row mb-3">
										<div class="col-md-12">
											...
											<textarea class="form-control shadow-sm" id="item_description" name="item_description" placeholder="Enter Item Description" rows="4"></textarea>
										</div>
									</div>

									<input type="hidden" id="VZ_app_user_id" name="VZ_app_user_id" value="<?php echo $this->session->userdata('user_id'); ?>">
						...
						<div id="collapseTwo" class="collapse" aria-labelledby="headingTwo" data-parent="#accordionExample">
							<div class="card-body">
								<form id="reProposalForm">
									...
									<div class="form-group">
										<label class="form-label" style="color: #000;" for="notes">Notes:</label>
						...
						<div id="collapseThree" class="collapse" aria-labelledby="headingTwo" data-parent="#accordionExample">
							<div class="card-body">
								<form id="editQuoteForm">
									...
									<div class="form-group">
										<label class="form-label" style="color: #000;" for="notes">Notes:</label>
										<textarea class="form-control" name="notes" id="notes2" rows="3" placeholder="Enter Notes"></textarea>
										<div class="error-message"></div>
									</div>
									<div class="form-group">
										<label class="form-label" style="color: #000;" for="valid-until">Valid Until:</label>
	...
	<script>
		...
		$(document).ready(function () {
			...
			$("#collapseFour").on("hidden.bs.collapse", function () {
				$("#addItemIcon").removeClass("bi-dash-circle").addClass("bi-plus-circle");
			});
		...
		document.getElementById('ItemForm').addEventListener('submit', function(e) {
			...
			const formData = {
				...
				VZ_app_user_id: document.getElementById('VZ_app_user_id').value
			};
			...
			Swal.fire({
				title: 'Creating Item...',
				text: 'Please wait while the item is being created in Zoho CRM.',
				allowOutsideClick: false,
				didOpen: () => {
					Swal.showLoading();
				}
			...
			fetch('<?php echo site_url('web/deals/create_item_in_zoho'); ?>', {
				method: 'POST',
				headers: {
			...
			.then(({ status, ok, body }) => {
				...
				if (body.success) {
					Swal.fire({
						icon: 'success',
						title: 'Item Created!',
						html: `${body.message}<br>Product ID: ${body.product_id}`,
						confirmButtonColor: '#d10908',
						confirmButtonText: 'OK'
					}).then(() => {
						document.getElementById('ItemForm').reset();
					});
				} else {
					let errorMessage = body.error || 'Failed to create item in Zoho CRM.';
					let isDuplicateError = false;

					// Check if the error mentions 'duplicate'
					if (errorMessage.toLowerCase().includes('duplicate') || (body.details && JSON.stringify(body.details).toLowerCase().includes('duplicate'))) {
						isDuplicateError = true;
					}

					if (body.errors) {
						errorMessage += '<br><ul>' + Object.values(body.errors).map(err => `<li>${err}</li>`).join('') + '</ul>';
					} else if (body.details) {
						errorMessage += `<br>Details: ${JSON.stringify(body.details)}`;
					}
					...
					if (isDuplicateError) {
						Swal.fire({
							...
							html: `
								It looks like a similar item already exists or is in the Recycle Bin.<br><br>
								<b>Tip:</b> Please check your CRM's Recycle Bin and delete the old record if needed, then try again.`,
							confirmButtonColor: '#d10908',
							confirmButtonText: 'OK'
						});
					} else {
			...
			.catch(error => {
				...
				Swal.fire({
					icon: 'error',
					title: 'Error',
					text: 'An unexpected error occurred while creating the item. Please try again later.',
					confirmButtonColor: '#d10908',
					confirmButtonText: 'OK'
				});
